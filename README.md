# mines3c - компилятор чего-то типа высокоуровнего в код программатора игры ШАХТЫ
...
- Безусловный цикл (`loop`). Пример ниже показывает, как сделать вечно
  крутящегося бота.
...
- Условный цикл (`while`). Выполняет действия только если условие верно. Пример:
...
- Условный оператор (`if` - `else`). Если условие верно --- выполняет ветку
`if`, иначе `else`. Примеры:
...
- Вызов функции. Все просто: имя, скобки, точка с запятой.

```rust
move_up();
```

- Вернуться из функции (`return`). Можно вернуть выражение, подобное тому, что
  можно передать как условие для `if` или `while`. Примеры:
...
- Напрямую вставить команду программатора (`__code__`). Пример:
```rust
...
- Сравнение значения переменной с числом. Число может быть заранее объявленной
  константой, или же явным значением. Примеры:
...
- Результат другой функции. Функция должна быть `inline` либо возвращать `arg` или `state`. Пример:
```rust
if low_hp() { ... }
```
